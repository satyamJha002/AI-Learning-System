...
const FlashcardManager = ({ documentId }) => {
  ...
  const [setToDetele, setSetToDelete] = useState(null);
  ...
  const handleGenerateFlashcards = async () => {
    ...
    try {
      ...
      toast.error("Failed to generate flashcard" || error.message);
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleToggleStar = async (cardId) => {
    try {
      ...
      toast.error("Failed to updated flashcard starred status.");
    }
  };
  ...
  const handleConfirmDelete = async () => {
    if (!setToDetele) return;
    setDeleting(true);
    try {
      await flashcardService.deleteFlashcardSet(setToDetele._id);
      toast.success("Flashcard set deleted successfully");
      setIsDeleteModalOpen(false);
      setSetToDelete(null);
      fetchFlashCardSets();
    } catch (error) {
      toast.error(error.message || "Failed to delete flashcard sets");
    } finally {
      setDeleting(false);
    }
  };
    ...
    if (flashcardSets.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-16 px-6">
          ...
          <button
            className="group inline-flex items-center gap-2 px-6 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text0-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
