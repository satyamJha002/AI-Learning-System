...
const Flashcard = ({ flashcard, onToggleStar }) => {
  ...
  return (
    <div className="relative w-full h-72" style={{ perspective: "1000px" }}>
      <div
        className={`relative w-full h-full transition-transform duratio-500 transform-gpu cursor-pointer`}
        style={{
      ...
      >
        ...
        <div
          className="absolute inset-0 w-full h-full bg-white/80 backdrop-blue-xl border-2 border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-8 flex flex-col justify-between"
          style={{
          ...
          <div className="flex items-start justify-between">
            ...
            <button
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${
                flashcard.isStarred
                  ? "bg-linear-to-br from-amber-400 to-yellow-500 text-white shadow-lg shadw-amber-500/25"
                  : "bg-slate-100 text-slate-100 hover:bg-slate-200 hover:text-amber-500"
              }`}
            >
              <Star
                className="w-4 h-4"
