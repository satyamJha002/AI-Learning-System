...
const Sidebar = ({isSidebarOpen, toggleSidebarOpen}) => {
  ...
  return (
    <>
      <div className={`fixed inset-0 bg-black/30 z-40 md:hidden transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={toggleSidebarOpen} aria-hidden="true">
      </div>
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 flex flex-col md:relative md:w-64 md:shrink-0 md:translate-x-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : '-translate-x-full'}`}>
        ...
        <div className='flex items-center justify-between h-16 px-5 border-b border-slate-200/60'>
          ...
          <button onClick={toggleSidebarOpen} className='md:hidden text-slate-500 hover:text-slate-800'>
            <X size={24}/>
          </button>
        </div>
        ...
        <nav className='flex-1 px-3 py-6 space-y-1.5'>
          {navLinks.map((link) => (
            <NavLink key={link.to} to={link.to} onClick={toggleSidebarOpen} className={({isActive}) => `group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 ${isActive ? 'bg-linear-to-r from-emerald-500 to-teal-500 text-white shadow-lg shadow-emerald-500/25' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900'}`}>
              {({isActive}) => (
                <>
