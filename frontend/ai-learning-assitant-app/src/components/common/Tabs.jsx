    ...
    <div className='w-full'>
      <div className='relative border-b-2 border-slate-100'>
        <nav className='flex gap-2'>
          {tabs.map((tab) => (
            <button className={`relative pb-4 px-2  md:px-6 text-sm font-semibold transition-all duration-200 ${activeTab === tab.name ? 'text-emerald-600' : 'text-slate-600 hover:text-slate-900'}`} key={tab.name} onClick={() => setActiveTab(tab.name)}>
