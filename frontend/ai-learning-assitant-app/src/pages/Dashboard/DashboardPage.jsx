...
const DashboardPage = () => {
  ...
  if(!dashbaordData || !dashbaordData.overview){
    return (
  ...
  const stats = [
    {
      ...
      value: dashbaordData.overview.totalDocuments,
      icon: FileText,
    ...
    },
    {
      ...
      value: dashbaordData.overview.totalFlashcards,
      icon: BookOpen,
    ...
    {
      label: 'Total Quizzes',
      value: dashbaordData.overview.totalQuizzes,
      icon: BrainCircuit,
      gradient: 'form-emerald-400 to-teal-500',
      ...
      <div className='relative max-w-7xl mx-auto'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-5'>
          {stats.map((stat, index) => (
            <div className='group relative bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-6 hover:shadow-2xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1' key={index}>
         ...
         <div className='bg-white/80 backdrop-blue-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-8'>
          ...
          {dashbaordData.recentActivity && (dashbaordData.recentActivity.documents.length > 0 || dashbaordData.recentActivity.quizzes.length > 0) ? (
            <div className='space-y-3'>
            {[
              ...(dashbaordData.recentActivity.documents || []).map(doc => ({
                id: doc._id,
              ...
              ...(dashbaordData.recentActivity.quizzes || []).map(quiz => ({
                id: quiz._id,
            ...
            .map((activity, index) => (
              <div className='group flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-200/60 hover:bg-white hover:border-slate-300/60 hover:shadow-md transition-all duratin-200' key={activity.id || index}>
