...
const DocumentListPage = () => {
  ...
  const handleUpload = async (e) => {
    ...
    if(!uploadFile || !uploadTitle) {
      toast.error('Please provide the file and title of the document.')
      return;
    }
    setUploading(true);
        ...
        <div className='relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8'>
          ...
          <form onSubmit={handleUpload} className='space-y-5'>
            ...
            <div className='space-y-2'>
              ...
              <input type="text" value={uploadTitle} onChange={(e)=>setUploadTitle(e.target.value)} required className='w-full h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10' placeholder='e.g., React Interview Prep' />
            </div>

            {/* File upload */}
            <div className='space-y-2'>
              <label className='block text-xs font-semibold text-slate-700 uppercase tracking-wide'>PDF file</label>
              <div className='relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 hover:border-emerald-400 hover:bg-emerald-50/30 transition-all duration-200/50'>
                ...
                <div className='flex flex-col items-center justify-center py-10 px-6'>
                  <div className='w-14 h-14 rounded-xl bg-linear-t-r from-emerald-100 to-teal-100 flex items-center justify-center mb-4'>
